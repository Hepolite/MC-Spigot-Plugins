...
public final class Time implements IValue
{
	...
	public static final long TICKS_PER_MINUTE = TICKS_PER_SECOND
		* SECONDS_PER_MINUTE;
	public static final long TICKS_PER_HOUR = TICKS_PER_MINUTE
		* MINUTES_PER_HOUR;
	...
	/**
	 * Constructs a new time representing the given amount of years, assuming
	 * {@code DAYS_PER_YEAR} days per year
	 * 
	...
	/**
	 * Constructs a new time from the provided string, taking in years, months,
	 * weeks, days, hours, minutes, seconds, and ticks. None of the quantities
	 * has to be specified, any one may be omitted. If no values are specified,
	 * an instant is returned.<br>
	 * <br>
	 * Example:<br>
	...
	public static Time fromString(final String string)
	{
		...
		for (final String part : string.split(" "))
			try
			{
				final long number = StringConverter
					.toLong(part.substring(0, part.length() - 1));
	...
	/**
	 * Decreases the time by a single tick to a minimum of 0 and returns the
	 * same instance
	 * 
	 * @return Returns the same instance as this method is invoked on
	 */
	...
	/**
	 * Decreases the time by the specified amount to a minimum of 0 and returns
	 * the same instance
	 * 
	 * @return Returns the same instance as this method is invoked on
	 */
	...
	/**
	 * @return The time represented measured in months, assuming
	 *         {@code DAYS_PER_MONTH} days per month
	 */
	...
	{
		config.set(property, toString());
	}
	@Override
	public void load(final IConfig config, final IProperty property)
	{
		time = fromString(config.getString(property)).time;
	}
