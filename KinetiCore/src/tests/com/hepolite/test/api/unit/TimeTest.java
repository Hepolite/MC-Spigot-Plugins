package com.hepolite.test.api.unit;
...
class TimeTest
{
	...
	void testIncrement()
	{
		...
		assertEquals(5,
			Time.fromInstant().increment(Time.fromTicks(5)).asTicks());
	}
	@Test
	void testDecrement()
	{
		...
		assertEquals(3,
			Time.fromTicks(5).decrement(Time.fromTicks(2)).asTicks());
		assertEquals(0,
			Time.fromTicks(5).decrement(Time.fromTicks(20)).asTicks());
	}
